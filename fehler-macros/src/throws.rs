...
pub fn entry(args: TokenStream, input: TokenStream) -> TokenStream {
    let ty = if args.is_empty() {
        ...
        syn::parse(args)
            .unwrap_or_else(|_| panic!("argument to #[throws] attribute must be a type"))
    };
    ...
    fn fold_return_type(&mut self, i: syn::ReturnType) -> syn::ReturnType {
        ...
        match i {
            ...
            syn::ReturnType::Type(arrow, ty) => {
                let result =
                    syn::parse2(quote::quote!(::std::result::Result<#ty, #error>)).unwrap();
...
fn modify_tail(is_unit_fn: bool, stmts: &mut Vec<syn::Stmt>) {
    let last_non_item_stmt = stmts
        .iter_mut()
        .rev()
        .filter(|s| {
            if let syn::Stmt::Item(_) = s {
                false
            } else {
                true
            }
    ...
    match last_non_item_stmt {
        Some(syn::Stmt::Expr(e)) if is_unit_fn => {
            ...
            stmts.push(syn::Stmt::Expr(
                syn::parse2(quote::quote!(::std::result::Result::Ok(()))).unwrap(),
            ));
        }
